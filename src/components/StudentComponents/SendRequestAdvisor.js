...
export default function SendRequestAdvisor({selectAdvisor, close}) {
	...
	return (
		<View style={styles.advisors}>
			<Text style={styles.titleAdvisors}>{<AntDesign name="closecircleo" onPress={() => close()} size={30} color={colors.red} />}  Cadastre seu trabalho</Text>
					...
					<ScrollView vertical={true}>
						...
						<KeyboardAvoidingView 
							...
							<TextInput
								...
								onContentSizeChange={(e) => setInputTitleHeight(e.nativeEvent.contentSize.height + 24)}
							/>
						</KeyboardAvoidingView>
						...
						style={{
							...
							<TextInput
								...
								numberOfLines={4}
								placeholderTextColor={colors.blackSpace}
								onContentSizeChange={(e) => setInputDescHeight(e.nativeEvent.contentSize.height + 24)}
							/>
						</KeyboardAvoidingView>
						</ScrollView>
				</SafeAreaView>
					<View style={{ justifyContent: 'space-between', width: '100%', alignItems: 'center', marginTop: 10, flexDirection:'row'}}>
						<View style={styles.inputAdvisors}>
							...
							/>
							<Text style={{color: colors.white, fontWeight: 'bold'}}>{selectAdvisor.NOME}</Text>
						</View>
						<View style={styles.buttonSend}>
							<Feather name="send" size={20} color="white" />
							<Text style={{color: colors.white, fontSize: 20, fontWeight:'bold', textAlign: 'center'}}>Enviar</Text>
						</View>
					</View>
			</View>
		</View>
