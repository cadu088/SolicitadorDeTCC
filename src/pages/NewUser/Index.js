...
export default function NewUser() {
  ...
  const [isSelf, setIsSelf] = useState({
    uri: "https://avatars.githubusercontent.com/u/72260079?v=4",
  });
  ...
  async function handlePostNewStudent() {
    if (camposPrenchidos()) {
      ...
      const data = {
        nome: txtName,
        iD_TIPO_PESSOA: isStudent ? 2 : 1,
        email: txtEmail,
        senha: txtPassword,
        ra: isStudent ? txtDocument : null,
        usuario: !isStudent ? txtDocument : null,
        img: isSelf.uri,
      };

      try {
        console.log(data);
        await api.post("/login/create", data).then(async (response) => {
          setLoading(true);
  ...
  function changeSwitch() {
    setIsSelf({
      uri: !isStudent
        ? "https://avatars.githubusercontent.com/u/72260079?v=4"
        : "https://sec.uniaraxa.edu.br/assets/lms/Pessoa/193-636687419898079554.jpg",
    });
  ...
  return (
    <>
      <SafeAreaView style={styles.container}>
        <View style={styles.content}>
          ...
          <View style={styles.switch}>
            <Text style={!isStudent ? styles.textActive : styles.textDisabled}>
              Orientador
            </Text>
            <Switch
      ...
      <PageLoader
        fadeIn={900}
        fadeOut={450}
        data={["Aguarde...", "Verificando Identidade", "Criando perfil"]}
        isActive={loading}
